...
local function applyFromTeleportData(player: Player, td: table)
        ...
        if td.session then
                ...
                setAttr("Role", normalizeRole(td.session.role)) -- "ROLE_TEACHER" 우선, fallback: "teacher"|"student"
...
local function enforceRoleOverride(player: Player)
        ...
        if Roles.isTeacherRole(roleValue) then
                player:SetAttribute("Role", Roles.TEACHER)
                return
        end
