...
local TeleportDataUtil = require(modules:WaitForChild("TeleportDataUtil"))

local function buildTeleportData(plr: Player, payload: any, sessionId: string)
        local tpData = TeleportRouterCore.buildDefaultTeleportData(payload, sessionId)
        tpData.meta = tpData.meta or {}
        tpData.meta.stage = "Stage1"
        tpData.meta.context = TeleportDataUtil.buildBase(plr)
        return tpData
end
