...
local function applyFromTeleportData(player: Player, td: table)
        ...
        if td.session then
                ...
                setAttr("roomCode", tostring(td.session.roomCode or ""))
                setAttr("userRole", tostring(td.session.role or ""))
        end
        if td.player then
...
local function enforceRoleOverride(player: Player)
        local userRole = player:GetAttribute("userRole")
        if Roles.isTeacherRole(userRole) then
